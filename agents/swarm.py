from crewai import Agent, Task, Crew, Process
from config import get_llm
from database.graph import market_graph
from tools.graph_tool import GraphTools
from tools.scraper_tool import ScraperTools
import json

class SomisSwarm:
    def __init__(self):
        # Initialize LLM (Picks up Groq if key present, else local)
        self.llm, self.engine_type = get_llm()
        
        # 1. The Harvester Agent (Data Intelligence)
        self.harvester = Agent(
            role='Financial Intelligence Specialist',
            goal='Extract entities and market impact from news text.',
            backstory="""You are a specialist in corporate intelligence. You parse news into clean summaries. 
            Identify the primary company or commodity mentioned and assign a score (-1.0 to 1.0).""",
            verbose=True,
            allow_delegation=False,
            # Tools removed from Harvester to prevent cloud validation issues
            llm=self.llm
        )

        # 2. The Knowledge Graph Architect (Topology Master)
        self.architect = Agent(
            role='Supply Chain Architect',
            goal='Map connections between global entities.',
            backstory="""You are a supply chain expert. You identify who is directly 
            connected to the affected entity (1st-order impact).""",
            verbose=True,
            allow_delegation=False,
            # Tools assigned strictly to Architect for Graph work
            tools=[GraphTools.update_graph_node, GraphTools.get_downstream_impact],
            llm=self.llm
        )

        # 3. The Ripple Agent (The Alpha Generator)
        self.ripple_agent = Agent(
            role='Second-Order Impact Analyst',
            goal='Identify non-obvious trade opportunities and final tickers.',
            backstory="""You are a master of indirect logic. You look for the "hidden" winners 
            that aren't mentioned in the news but are fundamentally affected. Always provide a reasoning chain.""",
            verbose=True,
            allow_delegation=False,
            # Second order tool for the money-maker
            tools=[GraphTools.get_second_order_impact],
            llm=self.llm
        )

    def process_news(self, news_text):
        """Processes news items through the swarm."""
        
        # Task 1: Extraction
        extraction_task = Task(
            description=f"Extract key entities (companies, commodities) and events from this news: {news_text}. Determine an impact score from -1.0 to 1.0.",
            agent=self.harvester,
            expected_output="A structured report identifying the main affected entity and its impact score."
        )

        # Task 2: Graph Update & Impact Tracing
        update_task = Task(
            description="""Update the internal knowledge graph for the identified entity using the tools. 
            Then, identify all immediate downstream companies or sectors that will be 
            impacted by this change. Ensure the impact score is recorded correctly.""",
            agent=self.architect,
            expected_output="A summary of the graph update and a list of direct downstream impacts."
        )

        # Task 3: Ripple Analysis & Trade Alert
        ripple_task = Task(
            description="""Analyze the second-order impacts of this event. 
            Identify a specific company (and its ticker) that is not directly mentioned but is linked via the 
            supply chain. 
            Generate an 'Opportunity Radar Alert' with a clear step-by-step reasoning chain 
            (use '->' as a separator).
            IMPORTANT: End your response with 'FINAL_TICKER: [TICKER]' (e.g., FINAL_TICKER: TATAMOTORS).""",
            agent=self.ripple_agent,
            expected_output="A detailed Alpha Alert with a multi-step reasoning chain and the final identified ticker."
        )

        # Execute Swarm
        crew = Crew(
            agents=[self.harvester, self.architect, self.ripple_agent],
            tasks=[extraction_task, update_task, ripple_task],
            process=Process.sequential,
            verbose=True
        )

        return crew.kickoff()

# Dummy Data for Hackathon Demo
DUMMY_FEED = [
    {"news": "Major Lithium mine in Western Australia flooded, production halted for 3 months."},
    {"news": "Tata Motors secures exclusive battery supply deal with CATL for new Nano EV line."},
    {"news": "Neon Gas shortage in Ukraine disrupts semiconductor laser etching worldwide."}
]
